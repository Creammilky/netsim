...
def create_lab_instance(G: nx.Graph, CURRENT_LAB_PATH):
    ...
    assign_id_for_routers(G, CURRENT_LAB_PATH)

    # generate config file according to IP and AS
    eth_table = define_network_interfaces(G, CURRENT_LAB_PATH)
...
def assign_id_for_routers(G: nx.Graph, CURRENT_LAB_PATH, vp_dafault_prefix="192.168.", as_default_prefix="10."):
    ...
    node_asn = nx.get_node_attributes(G, "as")  # 假设你节点上有 'asn' 属性（int）
